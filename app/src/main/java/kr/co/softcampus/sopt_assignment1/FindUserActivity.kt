...
class FindUserActivity : AppCompatActivity() {
    ...
    private fun initNetwork(){
        ...
        val call : Call<ResponseGetUserByEmailData> = ServiceCreator.getUserByEmailService.getgetUserByEmail(binding.etUseremail.text.toString())
        ...
        call.enqueue(object : Callback<ResponseGetUserByEmailData> {
            override fun onResponse(
                ...
                if(response.isSuccessful){
                    Log.d("success",response.body()?.message + response.body()?.data?.email+response.body()?.data?.name)
